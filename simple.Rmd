...
>- This is a **_null model_**. &nbsp; Are more informed guesses better?
...
>- Both variables are roughly normally distributed, which is not necessary at all.
>- Many interesting datasets are too small to organize into groups, compute group means, and observe the linear relationship so easily.
...
>- **Consequences**: $$ E(y \mid x) = \beta_{0} + \beta_{1}x \ , \ \operatorname{Var}(y \mid x) = \sigma^2$$
...
The **_mean standard error_**, $$\text{MSE} ~ = ~ \frac{ \text{RSS} }{ n-2 } ~ = ~ \frac{ \sum{ \left( y - \hat{y} \right)^2 } }{ n-2 } \ , $$
is an unbiased estimate of $\, \sigma^2 ~ = ~ \operatorname{Var}( \epsilon ) \,$.

>- The **_residual standard error_**, $\, \text{RSE} ~ = ~ \sqrt{\text{MSE}} \,$, is more practical; describes variability and compares models. 
...
>- If the regression model is correct, then at least 75% of responses will be within 2 RSEs of the regression line.
...
$R^2$ is the _relative_ reduction in variability gained by using the predictor instead of ignoring it. **Nothing more, nothing less!**

>- Beware: &nbsp; A true model can have a low $R^2$; an irrelevant model can have a large $R^2$.
...
>- Stop spreading the rumor that normality is required for linear regression.
